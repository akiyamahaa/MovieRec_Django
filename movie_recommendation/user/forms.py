...
def InvalidUser(value):
	if '@' in value or '+' in value or '-' in value:
		raise ValidationError('This is an invalid user, do not use these character: @, - , +')

def ExistUser(value):
	if User.objects.filter(username__iexact=value).exists():
		raise ValidationError('User with this this username already exists.')

def ExistEmail(value):
...
class SignupForm(forms.ModelForm):
	...
	def __init__(self, *args, **kwargs):
		...
		self.fields['username'].validators.append(ExistUser)
		self.fields['email'].validators.append(ExistEmail)
...
class ChangePasswordForm(forms.ModelForm):
	...
	class Meta:
		model = User
		fields = ('id', 'curr_password', 'new_password', 'confirm_password')
	...
	def clean(self):
		...
		if not user.check_password(curr_password):
			self._errors['curr_password'] = self.error_class(['Current password do not match. Please try again'])
